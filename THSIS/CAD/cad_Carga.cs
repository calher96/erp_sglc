...
namespace CAD
{
    public class cad_Carga
    {
        ...
        {
            List<ent_Carga> lista = new List<ent_Carga>();
            ResponseHelper response1 = null;
            string apiUrl = BasicVariable.webapi + "Carga/listar?tipo=" + tipo;
            ...
            using (HttpClient httpClient = new HttpClient())
            {
                try
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        {
                            //responseFinal.Response = (responseFinal.Response.ToString()).Substring(1, (responseFinal.Response.ToString()).Length - 2);
                            response1.response = JsonConvert.DeserializeObject<List<ent_Carga>>(responseFinal.Response.ToString());
                        }
            ...
            return (List<ent_Carga>)response1.response;
        }
    }
}
